...
type FlexConfig = {
  ...
  gap?: string
  containerAlign?: "start" | "end" | "center" | "stretch"
  containerJustify?: "start" | "end" | "center" | "between" | "around"
}
...
export default ((config: FlexConfig) => {
  const Flex: QuartzComponent = (props: QuartzComponentProps) => {
    ...
    const containerAlign = config.containerAlign ?? "center"
    const containerJustify = config.containerJustify ?? "start"

    return (
      <div
        class={classNames(props.displayClass, "flex-component")}
        style={`flex-direction: ${direction}; flex-wrap: ${wrap}; gap: ${gap}; align-items: ${containerAlign}; justify-content: ${containerJustify}; width: 100%;`}
      >
